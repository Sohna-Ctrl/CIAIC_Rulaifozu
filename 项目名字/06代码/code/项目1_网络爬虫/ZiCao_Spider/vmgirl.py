...
# @Time : ${20/08/08} ${18:00}
# @Author : 曹梓
# @Site : ${https://www.vmgirls.com/13344.html}
# @File : ${vmgirl}.py
# @Software: ${PyCharm}
...
def re_url(html, logger):
    ...
    try:
        ...
        # 解析网址
        img_urls = re.findall('<a href="(.*?)" alt=".*?" title=".*?">', html)
        logger.info("网页解析已完成！")
...
def get_photo(img_urls, dir_name, headers, save_path, logger):
    ...
    try:
        ...
        for img_url in img_urls:
            ...
            # 网址修正
            img_url = 'https://www.vmgirls.com/' + img_url
            print(img_url)
            # 请求网页
...
def save_data_init(save_path, logger):
    ...
    try:
        if not os.path.exists(save_path):
            ...
            # 写入合并单元格
            sheet.write_merge(0, 0, 0, 11, '保存图片的信息', style)
            sheet.write_merge(1, 1, 0, 11, '共计：', style)
            sheet.write_merge(2, 2, 0, 1, '存储位置', style)
            sheet.write_merge(2, 2, 2, 4, '文件名', style)
            sheet.write_merge(2, 2, 5, 11, '源链接', style)
            # 保存文件
            book.save(save_path)
...
def save_data(row, column, dir_name, file_name, img_url, img_num, img_count, save_path, logger):
    ...
    try:
        ...
        new_sheet.write_merge(row, row, column + 2, column + 4, file_name, style)
        new_sheet.write_merge(row, row, column + 5, column + 11,  img_url, style)
        # 追加图片总数
        if img_count == img_num:
            ...
            font.bold = False
            style.font = font
            new_sheet.write_merge(1, 1, 0, 11, '共计：' + str(img_num), style)
            pass
        # 保存文件
...
if __name__ == "__main__":
    ...
    # 保存图片和图片信息
    get_photo(img_urls, dir_name, headers, save_path, logger)
    pass
