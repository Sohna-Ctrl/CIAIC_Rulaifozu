...
var_cursor.execute("delete from t_original_Cosmetics_03")
...
try:
    def fun_web_analyse(webcontent):
        ...
        return var_html_namesource_text, var_html_price_text, var_html_website  # 返回价格,名称
...
def main():
    ...
    # 现在开始时第一步
        ...
        for j in range(len(sublist_web_name)):  # 以产品名称的个数为依据进行循环
            ...
            var_list_data=[sublist_web_name[j], var_normprice[j],sublist_web_website[j]]
            print(var_list_data)
            SqlInOut.fun_insert_method(var_list_data)
    pass
     ...
     #        worksheet_01.write(len(sublist_web_name) * i + j + 1, 2, sublist_web_website[j], style)
     #        workbook_test_new.save('.\\text.xls')
     #        # var_list_data.append(sublist_web_name[j])
     #        # var_list_data.append(sublist_web_website[j])
     #        worksheet_01.write(len(sublist_web_name) * i + j + 1, 1, var_normprice[j], style)
     #        workbook_test_new.save('.\\text.xls')
     #        var_list_data=[sublist_web_name[j], var_normprice[j],sublist_web_website[j]]
     #        print(var_list_data)
     #        SqlInOut.fun_insert_method(var_list_data)
